...
from Validations.Validations import empty_string_inside_widget
...
class ReceiptsPageController:
    def __init__(self, root, user_data, bg_color):
        ...
        self.list_of_user_receipts = self.receipts_page_model.receipt_list_from_database(self.user_data[0])
        self.receipts_page_view.receipt_combobox_update(
            self.list_of_user_receipts)
    ...
    def add_new_receipt(self):
        ...
        AddNewReceiptController(self.root, self.user_data, self.bg_color, self.items_df, self.list_of_user_receipts,
                                state="ADD", receipt_name=None)

    # Delete receipt button method
    def delete_receipt(self, receipt_name: str):
    ...
    def choose_receipt(self, receipt_name: str):
    ...
    def edit_receipt(self, receipt_name: str):
        if not empty_string_inside_widget(receipt_name):
            from ReceiptsPage.AddNewReceipt.AddNewReceiptController import AddNewReceiptController
            self.receipts_page_view.destroy_receipts_page_frame()
            AddNewReceiptController(self.root, self.user_data, self.bg_color, self.items_df, self.list_of_user_receipts,
                                    state="UPDATE", receipt_name=receipt_name)
            return
