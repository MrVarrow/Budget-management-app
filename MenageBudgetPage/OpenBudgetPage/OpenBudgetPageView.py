...
class OpenBudgetView:
    def __init__(self, master, controller, bg_color):
        ...
        self.income_frame.grid(row=2, column=0)

        # Expenses frame
        self.expenses_frame = Frame(self.open_budget_frame, bg=self.bg_color)
        self.expenses_frame.grid(row=2, column=1)

        # Labels
        Label(self.open_budget_frame, text="Your monthly budget", font=('Arial', 40), bg='light gray') \
            .grid(row=0, column=0, columnspan=2, sticky=EW, padx=190, pady=40, ipadx=210, ipady=50)

        Label(self.open_budget_frame, text="date", font=('Arial', 20)) \
            .grid(row=1, column=0, columnspan=2)

        Label(self.open_budget_frame, text="total income", font=('Arial', 20)) \
            .grid(row=3, column=0)

        Label(self.open_budget_frame, text="total expenses", font=('Arial', 20)) \
            .grid(row=3, column=1)

        Label(self.open_budget_frame, text="Free amount: ", font=('Arial', 20)) \
            .grid(row=4, column=0, columnspan=2)

        # Back to manage budget page button
        Button(self.open_budget_frame, text="back", font=('Arial', 15), bg='light gray',
               command=lambda: self.destroy_budget_frame()) \
            .grid(row=5, column=0, columnspan=2)

        # Income treeview
        self.income_table = ttk.Treeview(self.income_frame, height=15)
        self.income_table.pack(fill="both", expand=True, side=LEFT)
        ...
        # Expenses treeview
        self.expenses_table = ttk.Treeview(self.expenses_frame, height=15)
        self.expenses_table.pack(fill="both", expand=True, side=LEFT)
