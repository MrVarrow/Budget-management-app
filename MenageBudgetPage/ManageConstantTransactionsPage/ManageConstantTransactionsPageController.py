...
class ManageConstBudgetController:
    def __init__(self, root, user_data, bg_color, month_date):
        ...
        self.manage_const_budget_view = ManageConstBudgetView(self.root, self, self.bg_color, self.incomes_df, self.expenses_df, self.month_date)
    ...
    def delete_income(self, index):
        try:
            result = messagebox.askquestion("Warning", "Do you want to delete selected item from your constant incomes?")
            if result == "yes":
    ...
    def delete_expense(self, index):
        try:
            result = messagebox.askquestion("Warning", "Do you want to delete selected item from your constant expenses?")
            if result == "yes":
                self.incomes_df = self.manage_const_budget_model.delete_from_expenses_df(index, self.expenses_df)
                self.manage_const_budget_view.clear_expenses()
                self.manage_const_budget_view.add_items_to_expenses(self.expenses_df)
        except IndexError:
            messagebox.showinfo("Information", "Please select item to delete first, by clicking on it.")
    ...
    def update_transactions(self):
        self.manage_const_budget_model.add_items_to_database(self.user_data, self.incomes_df, self.expenses_df)
